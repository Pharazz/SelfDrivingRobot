...
class myRobot(Robot):
    ...
    def inverse_kinematics(self, p_dot, theta):
        ...
        #print("This is the trig array for:",trig)
        #print("p_dot inv kin",p_dot)
        #print(theta*180/3.14)
        #print("        ")
        wheel_angular_velocities = np.dot(trig, p_dot)
        #print("wheel vel:",wheel_angular_velocities)
    ...
    def forward_kinematics(self, wheel_angular_velocities, theta):
       ...
       #print("trig",trig)
       #print("curr w",wheel_angular_velocities)
       trig = trig*(wheel_radius/3)
       #print("trig",trig)
       #p_dot = np.dot(trig, np.transpose(wheel_angular_velocities))
       p_dot = np.dot(trig, wheel_angular_velocities)
       print(p_dot)
       #p_dot = p_dot*(wheel_radius/3)
       #rint(p_dot)
       #print("pfinal", p_dot) 
        #
    ...
    def motor_sat(self, wheel_angular_velocities, limit_value):	
       #print(limit_value)
       #print(wheel_angular_velocities)
       #np.clip(wheel_angular_velocities, (-1*limit_value), limit_value)
